...
### 2-4. 이슈 및 해결방안
**프론트엔드**
* 사용자의 목소리와 AI의 음성 답변이 서버에 제대로 전달되지 않는 현상: 음성 데이터를 binary로 변환 후, 한번에 데이터를 전송하는 방법으로 해결
* AI의 음성 답변을 프론트엔드를 통해 전달을 받았지만 음성이 재생되지 않는 현상: 음성 데이터를 binary로 변환 후, 1,024bytes 단위의 청크 배열로 나누어 저장한 뒤 순서대로 재생하는 방법으로 해결
